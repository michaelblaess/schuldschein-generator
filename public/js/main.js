...
// For full source, see original js/main.js in repo.
...
function generiereVertragstext(d) {
    ...
    let auszahlungsText = '';
    if (d.auszahlungsart === 'bar') { auszahlungsText = trans['text-in-cash']; }
    else if (d.auszahlungsart === 'ueberweisung') { auszahlungsText = trans['text-by-transfer'] + (d.nehmer_iban ? ' ' + trans['text-to-account'] + ' <span data-preview="nehmer_iban">' + formatiereIBAN(d.nehmer_iban) + '</span>' : ''); }
    else if (d.auszahlungsart === 'paypal') { auszahlungsText = trans['text-by-paypal']; }
    else if (d.auszahlungsart === 'sonstiges') { auszahlungsText = trans['text-by-other']; }
    ...
    return `
        <div class="space-y-4">
            ...
            <h2 class="font-bold mt-4 mb-2">§ ${d.zweck ? '4' : '3'} ${trans['section-disbursement']}</h2>
...
// Copy preview text (legacy variant used by static page)
