// Public copy of jsPdfController (kept in sync with js/jsPdfController.js)
(function () {
  'use strict';
  ...
  window.generatePDF = function generatePDF() {
    ...
    if (!jsPDF) { alert('PDF-Bibliothek (jsPDF) nicht gefunden.'); return; }
    ...
    const lh = 16;
    lines.forEach(line => {
      if (cursorY + lh > pageHeight - margin) { doc.addPage(); cursorY = margin; }
